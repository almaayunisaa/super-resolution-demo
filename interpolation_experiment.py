...
dataset = os.listdir(r"dataset\skin lesion\lowered")

for img_name in dataset:
    path = os.path.join("dataset\skin lesion\lowered", img_name)
    ...
    cv2.imwrite(f"dataset/skin lesion/interpolation_experiment/nearest/{img_name}", cv2.cvtColor(img_result, cv2.COLOR_RGB2BGR))
    img_result = bilinear_interpolation(img, target_size)
    cv2.imwrite(f"dataset/skin lesion/interpolation_experiment/bilinear/{img_name}",cv2.cvtColor(img_result, cv2.COLOR_RGB2BGR))
    img_result = bicubic_interpolation(img, target_size, -0.5)
    cv2.imwrite(f"dataset/skin lesion/interpolation_experiment/bicubic/-0.5/{img_name}",cv2.cvtColor(img_result, cv2.COLOR_RGB2BGR))
    img_result = bicubic_interpolation(img, target_size, -0.75)
    cv2.imwrite(f"dataset/skin lesion/interpolation_experiment/bicubic/-0.75/{img_name}", cv2.cvtColor(img_result, cv2.COLOR_RGB2BGR))
    img_result = bicubic_interpolation(img, target_size, -1.0)
    cv2.imwrite(f"dataset/skin lesion/interpolation_experiment/bicubic/-1.0/{img_name}", cv2.cvtColor(img_result, cv2.COLOR_RGB2BGR))